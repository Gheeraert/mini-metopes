# Métadonnées JSON v2 (schema_version 2.0)
...
Remplace `metadata-json-v1.md`. La compatibilité avec les JSON v1 n'est pas
assurée : un JSON v1 est rejeté (`unsupported_schema_version`).
...
Exemples : `tests/fixtures/metadata/minimal-v0.1.json` (minimal),
`complete-purh-v0.2.json` (complet PURH), `multilingual-v0.2.json`
...
- **Langues** : syntaxe BCP 47 raisonnable (`fr`, `fr-FR`, `en-GB`, `la`,
  ...
  l'inverse.
- **Dates** : `AAAA`, `AAAA-MM`, `AAAA-MM-JJ` ; jamais complétées.
